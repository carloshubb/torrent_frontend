...
const torrents = ref({})
const head_title = ref('TRENDING TORRENTS LAST 24 HOURS')
...
async function fetchTorrents() {
    try {
        const response = await torrentService.get('/torrents/type?type=trending-d-' + type);
        // For demo, assuming all torrents in one group with title "All Torrents"
        if (response?.data?.data?.length > 0) {
            head_title.value = response.data.data[0].category_title
        }
        torrents.value = {
            icon: icon,
            title: head_title.value,
