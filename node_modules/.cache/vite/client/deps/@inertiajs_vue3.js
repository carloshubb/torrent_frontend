...
function baseIntersection(arrays, iteratee2, comparator) {
  ...
  outer:
    while (++index < length && result2.length < maxLength) {
      ...
      value = comparator || value !== 0 ? value : 0;
      if (!(seen ? cacheHas_default(seen, computed4) : includes2(result2, computed4, comparator))) {
        othIndex = othLength;
        while (--othIndex) {
          var cache = caches[othIndex];
          if (!(cache ? cacheHas_default(cache, computed4) : includes2(arrays[othIndex], computed4, comparator))) {
            continue outer;
          }
        }
        if (seen) {
...
function baseExtremum(array, iteratee2, comparator) {
  ...
  while (++index < length) {
    ...
    if (current != null && (computed4 === void 0 ? current === current && !isSymbol_default(current) : comparator(current, computed4))) {
...
function baseSortedIndexBy(array, value, iteratee2, retHighest) {
  ...
  while (low < high) {
    var mid = nativeFloor4((low + high) / 2), computed4 = iteratee2(array[mid]), othIsDefined = computed4 !== void 0, othIsNull = computed4 === null, othIsReflexive = computed4 === computed4, othIsSymbol = isSymbol_default(computed4);
    if (valIsNaN) {
...
function baseSortedIndex(array, value, retHighest) {
  ...
  if (typeof value == "number" && value === value && high <= HALF_MAX_ARRAY_LENGTH) {
    while (low < high) {
      ...
      if (computed4 !== null && !isSymbol_default(computed4) && (retHighest ? computed4 <= value : computed4 < value)) {
        low = mid + 1;
      } else {
        high = mid;
      }
...
function baseSortedUniq(array, iteratee2) {
  ...
  while (++index < length) {
    var value = array[index], computed4 = iteratee2 ? iteratee2(value) : value;
    if (!index || !eq_default(computed4, seen)) {
      var seen = computed4;
...
function objectToFormData(source, form = new FormData(), parentKey = null) {
  ...
  for (const key2 in source) {
    if (Object.prototype.hasOwnProperty.call(source, key2)) {
      append(form, composeKey(parentKey, key2), source[key2]);
    }
  }
  return form;
}
function composeKey(parent2, key2) {
  return parent2 ? parent2 + "[" + key2 + "]" : key2;
}
function append(form, key2, value) {
  if (Array.isArray(value)) {
    return Array.from(value.keys()).forEach((index) => append(form, composeKey(key2, index.toString()), value[index]));
...
var CurrentPage = class {
  ...
  set(page22, {
    ...
    return this.resolve(page22.component).then((component2) => {
      ...
      const location = typeof window !== "undefined" ? window.location : new URL(page22.url);
      ...
      return new Promise((resolve) => {
        ...
        return this.swap({ component: component2, page: page22, preserveState }).then(() => {
          if (!preserveScroll) {
            Scroll.reset();
          }
  ...
  setQuietly(page22, {
    ...
    return this.resolve(page22.component).then((component2) => {
      ...
      return this.swap({ component: component2, page: page22, preserveState });
    });
  }
  clear() {
    this.cleared = true;
  ...
  swap({
    ...
    return this.swapComponent({ component: component2, page: page22, preserveState });
  }
  resolve(component2) {
...
var History = class {
  ...
  pushState(page22, cb = null) {
    ...
    queue.add(() => {
      return this.getPageData(page22).then((data) => {
        const doPush = () => {
          this.doPushState({ page: data }, page22.url);
          cb && cb();
        };
        if (isChromeIOS) {
          setTimeout(doPush);
  ...
  getPageData(page22) {
    return new Promise((resolve) => {
      return page22.encryptHistory ? encryptHistory(page22).then(resolve) : resolve(page22);
    });
  }
  processQueue() {
  ...
  replaceState(page22, cb = null) {
    ...
    queue.add(() => {
      return this.getPageData(page22).then((data) => {
        const doReplace = () => {
          this.doReplaceState({ page: data }, page22.url);
          cb && cb();
        };
        if (isChromeIOS) {
...
var RequestParams = class _RequestParams {
  ...
  setPreserveOptions(page22) {
    this.params.preserveScroll = this.resolvePreserveOption(this.params.preserveScroll, page22);
    this.params.preserveState = this.resolvePreserveOption(this.params.preserveState, page22);
  }
  runCallbacks() {
  ...
  resolvePreserveOption(value, page22) {
    if (typeof value === "function") {
      return value(page22);
    }
    if (value === "errors") {
  ...
  const form = reactive({
    ...
    submit(...args) {
      ...
      if (method2 === "delete") {
        ...
        router[method2](url, data2, _options);
      }
    },
    get(url, options) {
...
function usePoll(interval, requestOptions = {}, options = {
  ...
  const { stop, start: start3 } = router.poll(interval, requestOptions, {
    ...options,
    autoStart: false
  });
  onMounted3(() => {
