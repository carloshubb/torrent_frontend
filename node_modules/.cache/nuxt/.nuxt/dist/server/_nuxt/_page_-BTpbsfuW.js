import { H as Head } from "./components-CkKCuKad.js";
import { _ as __nuxt_component_1 } from "./TorrentHome-CuBGNTUQ.js";
import { ref, withCtx, unref, createVNode, createBlock, createCommentVNode, openBlock, Fragment, renderList, createTextVNode, toDisplayString, useSSRContext } from "vue";
import { ssrRenderComponent, ssrRenderList, ssrRenderAttr, ssrRenderClass, ssrInterpolate } from "vue/server-renderer";
import _sfc_main$1 from "./AppLayout-YcpLZ4jN.js";
import { T as TorrentPagination } from "./TorrentPagination-DsBJutVk.js";
import "./torrentService-CHqQEqPf.js";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/hookable/dist/index.mjs";
import { d as useRoute } from "../server.mjs";
import { u as useHead } from "./composables-Dj0Rik_j.js";
import "vue-router";
import "@inertiajs/vue3";
import "./useAuth-COd_Gpgv.js";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/ofetch/dist/node.mjs";
import "#internal/nuxt/paths";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/unctx/dist/index.mjs";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/h3/dist/index.mjs";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/radix3/dist/index.mjs";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/defu/dist/defu.mjs";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/ufo/dist/index.mjs";
import "axios";
import "D:/_4_Work/Web/torrent-nuxt/frontend/node_modules/@unhead/vue/dist/index.mjs";
const _sfc_main = {
  __name: "[page]",
  __ssrInlineRender: true,
  setup(__props) {
    const torrents = ref({});
    ref("");
    const route = useRoute();
    route.params.category;
    const topLinks = ref([]);
    route.params.page;
    const currentPage = ref(1);
    const lastPage = ref(1);
    useHead({
      title: "1331x - Ultimate Torrent Hub",
      meta: [
        { name: "description", content: "Your ultimate destination for high-quality torrents and streaming content" }
      ]
    });
    return (_ctx, _push, _parent, _attrs) => {
      const _component_Head = Head;
      const _component_TorrentHome = __nuxt_component_1;
      _push(`<!--[-->`);
      _push(ssrRenderComponent(_component_Head, { title: "Trending Torrents" }, null, _parent));
      _push(ssrRenderComponent(_sfc_main$1, null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="space-y-4 mt-5"${_scopeId}><div class="flex flex-wrap gap-2 bg-gray-300 p-2 rounded"${_scopeId}><!--[-->`);
            ssrRenderList(unref(topLinks), (item, index) => {
              _push2(`<a${ssrRenderAttr("href", `/sub/${item.id}/1/`)} class="btn-icon"${_scopeId}><i class="${ssrRenderClass(item.icon)}"${_scopeId}></i> ${ssrInterpolate(item.name)}</a>`);
            });
            _push2(`<!--]--></div></div>`);
            if (Object.keys(unref(torrents)).length > 0) {
              _push2(ssrRenderComponent(_component_TorrentHome, {
                key: "trending",
                torrents: unref(torrents),
                flag_edit: true
              }, null, _parent2, _scopeId));
            } else {
              _push2(`<!---->`);
            }
            if (unref(lastPage) > 1) {
              _push2(ssrRenderComponent(TorrentPagination, {
                currentPage: unref(currentPage),
                lastPage: unref(lastPage),
                onPageChanged: _ctx.onPageChange
              }, null, _parent2, _scopeId));
            } else {
              _push2(`<!---->`);
            }
          } else {
            return [
              createVNode("div", { class: "space-y-4 mt-5" }, [
                createVNode("div", { class: "flex flex-wrap gap-2 bg-gray-300 p-2 rounded" }, [
                  (openBlock(true), createBlock(Fragment, null, renderList(unref(topLinks), (item, index) => {
                    return openBlock(), createBlock("a", {
                      key: index,
                      href: `/sub/${item.id}/1/`,
                      class: "btn-icon"
                    }, [
                      createVNode("i", {
                        class: item.icon
                      }, null, 2),
                      createTextVNode(" " + toDisplayString(item.name), 1)
                    ], 8, ["href"]);
                  }), 128))
                ])
              ]),
              Object.keys(unref(torrents)).length > 0 ? (openBlock(), createBlock(_component_TorrentHome, {
                key: "trending",
                torrents: unref(torrents),
                flag_edit: true
              }, null, 8, ["torrents"])) : createCommentVNode("", true),
              unref(lastPage) > 1 ? (openBlock(), createBlock(TorrentPagination, {
                key: 1,
                currentPage: unref(currentPage),
                lastPage: unref(lastPage),
                onPageChanged: _ctx.onPageChange
              }, null, 8, ["currentPage", "lastPage", "onPageChanged"])) : createCommentVNode("", true)
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`<!--]-->`);
    };
  }
};
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/cat/[category]/[page].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
//# sourceMappingURL=_page_-BTpbsfuW.js.map
